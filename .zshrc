...
fpath+=~/.asciiArt        #ascii art functions
#autoload ascii art functions
autoload zshAscii 

# Load zsh splash on startup
zshAscii $info_color $version $verDate
echo ${sysInfo}' \n\n'
...
configure_prompt() {
   ...
   # bold_style='\e[1m'
   # normal_style='\e(B\e[m'
    
    # check if user is root
    if [ "$EUID" -eq 0 ]; then
      #change prompt to denote root user 
      info_color='%F{1}'  #Red Info
      prompt_symbol=💀
    fi
    case "$PROMPT_ALTERNATIVE" in
        twoline)
          PROMPT=$prompt_color'┌──${debian_chroot:+($debian_chroot)─}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))─}(%B'$info_color'%n'$prompt_symbol$'%m%b'$prompt_color')-[%B'$path_color'%(6~.%-1~/…/%4~.%5~)%b'$prompt_color']'$nL'└─'$error_color'%B%(?..'$error_color' [%?] )'$info_color'%(!.#.$)%b%F{reset} '
